...
while ($row = $statsAndCharacters->fetch_assoc()) {
    ...
    echo <input type="hidden" name="stats_id" value="<?php echo $stats_id['stats_id']; ?>">
     echo   <input type="hidden" name="actionType" value="Delete">
      echo  <a href="#" class="btn btn-danger" onclick="if(confirm('Are you sure?')) { this.closest('form').submit(); }">
